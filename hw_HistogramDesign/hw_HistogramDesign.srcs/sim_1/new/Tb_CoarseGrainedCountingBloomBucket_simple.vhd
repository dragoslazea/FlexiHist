...
entity Tb_CoarseGrainedCountingBloomBucket_simple is
end Tb_CoarseGrainedCountingBloomBucket_simple;

architecture Tb of Tb_CoarseGrainedCountingBloomBucket_simple is

component CountingBloomFilterBucket is
    Generic (
        BUCKET_ID : integer := 0;
        NUM_HASHES : integer range 2 to 5;
        NUM_COUNTERS_BITS : integer range 1 to 32 := 8
    );
    Port ( 
        -- inputs
        aclk : in STD_LOGIC;
...
nrst <= '0', '1' after 4 * T;
        ...
        if rising_edge(clk) and nrst = '1' then 
            if configured_buckets = total_buckets then
                if end_of_reading = '1' then
                    opcode <= "0011";
                    if test_val <= max and opcode = "0011" then
                        test_val <= test_val + 1;
                    end if;
                else 
                    opcode <= "0010";
    ...
    --opcode <= "0010" when configured_buckets = total_buckets and end_of_reading = '0' and nrst = '1' else
     --         "0011" when configured_buckets = total_buckets and end_of_reading = '1' and nrst = '1' else 
     --         "0001";
     
--     opcode <= "0001" when configured_buckets < total_buckets else
--               "0010" when end_of_reading = '0' and nrst = '1' else
--               "0011";
...
--                low <= low + size;
    ...
    histo : CountingBloomFilterBucket
        generic map (NUM_COUNTERS_BITS => 4, NUM_HASHES => 2)
        port map (
            aclk => clk,
            nrst => nrst,
    ...
    process (clk)
        file results : text open write_mode is "C:\IVA\Research\histograms\HybridHistogramsOnStreamingData\results_counting_bloom_simple.csv";
