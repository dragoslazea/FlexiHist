...
entity Tb_FineGrainedBRAMBucket is
end Tb_FineGrainedBRAMBucket;

architecture Tb of Tb_FineGrainedBRAMBucket is
...
begin
    ...
    low <= x"00000000";
    --high <= x"00002800";
    high <= x"000012FD"; -- 4861
    max <= x"000012FD"; -- 4861
    
    
    in_data <=  opcode & x"0000000000000000" when opcode = "0000" else
                opcode & max & low when opcode = "0001" else     
...
--                low <= low + size;
    ...
    process (clk)
        file results : text open write_mode is "C:\IVA\Research\histograms\HybridHistogramsOnStreamingData\results_in_memory.csv";
