...
entity Tb_HybridHistogramDataFromFile is
end Tb_HybridHistogramDataFromFile;

architecture Tb of Tb_HybridHistogramDataFromFile is

component HybridHistogram is
    Generic (
        NUM_COARSE_SIMPLE_BUCKETS : integer := 5;
        NUM_COARSE_DISTINCT_BUCKETS : integer := 5;
        NUM_COARSE_BLOOM_BUCKETS : integer := 5;
        NUM_COARSE_COUNTING_BLOOM_BUCKETS : integer := 5;
        NUM_FINE_BUCKETS : integer := 5;
...
signal high : STD_LOGIC_VECTOR (31 downto 0) := x"000000DC";
...
signal total_buckets : integer := 22;
...
--                    opcode <= "0100";
    ...
    histo : HybridHistogram
        generic map (
            NUM_COARSE_SIMPLE_BUCKETS => 4,
            NUM_COARSE_DISTINCT_BUCKETS => 4,
            NUM_COARSE_BLOOM_BUCKETS => 4,
            NUM_COARSE_COUNTING_BLOOM_BUCKETS => 4,
            NUM_FINE_BUCKETS => 4,
            NUM_SUB_BUCKETS => 10
        )
        port map (
    ...
    process (clk)
        file results : text open write_mode is "C:\IVA\Research\histograms\HybridHistogramsOnStreamingData\results_hybrid_5_types_of_buckets.csv";
