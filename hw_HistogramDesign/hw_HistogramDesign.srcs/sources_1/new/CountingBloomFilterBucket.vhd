...
entity CountingBloomFilterBucket is
    Generic (
        BUCKET_ID : integer := 0;
...
end CountingBloomFilterBucket;

architecture Arch of CountingBloomFilterBucket is
...
constant NUM_COUNTERS : integer := 2 ** NUM_COUNTERS_BITS;
...
type bloom_count_array_type is array (0 to NUM_COUNTERS - 1) of std_logic_vector (31 downto 0);
...
signal bloom_counts : bloom_count_array_type := (others => (others => '0'));
...
begin
    ...
    -- opcode = 0100 {SET ID)
    process(aclk)
        variable min_count : std_logic_vector(31 downto 0) := (others => '1');
    begin
            ...
            if nrst = '1' then
                if s_axis_in_tvalid = '1' and data_ready = '1' then
                    case s_axis_in_tdata(67 downto 64) is
                        ...
                        when "0010" =>
                            -- update + insert into the counting bloom filter
                            if is_configured = '1' and s_axis_in_tdata(31 downto 0) >= low and s_axis_in_tdata(31 downto 0) < high then
                                ...
                                -- insert into the counting bloom filter
                                for i in 0 to NUM_HASHES - 1 loop
                                    bloom_counts(to_integer(unsigned(hashes(i)(NUM_COUNTERS_BITS - 1 downto 0)))) <= bloom_counts(to_integer(unsigned(hashes(i)(NUM_COUNTERS_BITS - 1 downto 0)))) + 1;
                                end loop;
                        ...
                        when "0011" =>
                            ...
                            if is_configured = '1' and s_axis_in_tdata(31 downto 0) >= low and s_axis_in_tdata(31 downto 0) < high then
                                ...
                                min_count := bloom_counts(to_integer(unsigned(hashes(0)(NUM_COUNTERS_BITS - 1 downto 0))));
                                
                                for i in 1 to NUM_HASHES - 1 loop
                                    if bloom_counts(to_integer(unsigned(hashes(i)(NUM_COUNTERS_BITS - 1 downto 0)))) < min_count then
                                        min_count := bloom_counts(to_integer(unsigned(hashes(i)(NUM_COUNTERS_BITS - 1 downto 0))));
                                    end if;
                                end loop;
                                report "min = " & integer'image(to_integer(unsigned(min_count)));
                                data(63 downto 32) <= min_count;
                            else 
                                data <= s_axis_in_tdata;
                            end if;
                        when "0100" =>
